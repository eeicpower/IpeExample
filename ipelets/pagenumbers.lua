...
local appendix_layer = prefix .. "_appendix"

local pagenr_wildcard = "%[page%]" -- ignore the %
local totalnr_wildcard = "%[total%]" -- ignore the %
...
function print_on_every_page(model, pagenr_objects)
	...
	-- count total numbered pages
	local totalnr = 0
	for i = 1, #doc do
		local p = doc[i]
		-- stop counting if the current page contains the appendix layer
		if page_has_layer(p, appendix_layer) then
			break
		end

		-- increase the totalnr, if the current page does not contain the
		-- dont_count layer
	...
	for i = 1, #doc do
		...
		for j = 1, #pagenr_objects do
			local repl_text1 = clones[i][j]:text():gsub(pagenr_wildcard, pagenr)
			local repl_text2 = repl_text1:gsub(totalnr_wildcard, totalnr)
			clones[i][j]:setText(repl_text2)
...
----------------------------------------------------------------------
-- show some message if someone clicks in the ipelet menu -------------
function run()
   ipeui.messageBox(nil,
		"information",
		"Nothing to do here.",
		[[Information on how to use this ipelet can be found in the GitHub.
